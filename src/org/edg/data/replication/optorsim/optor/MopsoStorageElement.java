...
import java.util.*;
import org.edg.data.replication.optorsim.infrastructure.BinaryTree;
...
/**
 * A StorageElement where the file to delete is chosen using a Least Frequently
 * Used (LFU) algorithm.
 * <p>
 * <p>
 * @author leobusta
 * @since JDK1.6
 */
public class MopsoStorageElement extends AccessHistoryStorageElement {

    /**
     * @param site The GridSite on which the SE is situated.
     * @param capacity The total capacity of this SE.
     */
    public MopsoStorageElement(GridSite site, long capacity) {
        super(site, capacity);
    }
    ...
    /**
     * Returns the DataFiles which have been accessed least frequently in recent
     * access history. If there is a file on the SE which is not in the access
     * history this is used first since it has been accessed the least number of
     * times (ie zero).
     *
    ...
    public List filesToDelete(DataFile newFile) {
        ...
        while (!nonAHFiles.isEmpty()) {
            ...
            if (deleteableFileSize >= newFile.size()) {
